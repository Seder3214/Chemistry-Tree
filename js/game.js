...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		let amt = layer=="pt"?player[layer].points.add(1).plus((canMax&&tmp[layer].baseAmount.gte(tmp[layer].nextAt))?tmp[layer].resetGain:0).div(tmp[layer].directMult):player[layer].points.plus((canMax&&tmp[layer].baseAmount.gte(tmp[layer].nextAt))?tmp[layer].resetGain:0).div(tmp[layer].directMult)
